...
**/
   ...
   // Highエントリー
   if( price <= sigma00 && sigma >= MinSigma && ( rsi <= RsiL || RsiU <= rsi ) && ( !CciLimit || cci <= -CciAbs ) &&  ( !Envelopes || price <= envelopesDown ) && lengthD >= MinLength && ( !Vgfx || ( vgfxBuy != 0 && vgfxBuy != EMPTY_VALUE ) ) && ( !Sto || stochastic ) && ( !HiLoBands || ( High[1] >= hiBand ) ) ) {
      if( spread > MaxSpread ) {
         if( lastLog != Time[0] ) {
   ...
   // Lowエントリー
   if( price >= sigma00 && sigma >= MinSigma && ( rsi <= RsiL || RsiU <= rsi ) && ( !CciLimit || cci >= CciAbs ) && ( !Envelopes || price >= envelopesUp ) && lengthD >= MinLength && ( !Vgfx || ( vgfxSell != 0 && vgfxSell != EMPTY_VALUE ) ) && ( !Sto || stochastic ) && ( !HiLoBands || ( Low[1] <= loBand ) ) ) {
      if( spread > MaxSpread ) {
         if( lastLog != Time[0] ) {
            Print( "Invalid Spread.["+spread+"]" );
            lastLog = Time[0];
